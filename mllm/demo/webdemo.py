...
from mllm.models.builder.build_llava import load_pretrained_llava, load_pretrained_llavaintern
...
parser.add_argument('--base_model', default='llama', choices=['llama', 'intern'])
...
model_args = Config(dict(
    type='llava' if args.base_model == 'llama' else 'llavaintern',
    version='v1',
    ...
    process_func_args=dict(
        ...
        text=dict(type='LlavaTextProcessV1') if args.base_model == 'llama' else dict(type='LlavaInternTextProcessor'),
        image=dict(type='LlavaImageProcessorV1'),
    ),

    conv_args=dict(
        conv_template='vicuna_v1.1' if args.base_model == 'llama' else 'intern',
...
load_pretrained_func = load_pretrained_llava if args.base_model == 'llama' else load_pretrained_llavaintern
model, preprocessor = load_pretrained_func(model_args, training_args, **quantization_kwargs)
...
"""
                ...
                video_file_url = os.path.join(os.path.dirname(__file__), f"examples/petal_20230711_153216_Compressed.mp4")
                gr.Video(value=video_file_url, interactive=False, width=600)
            with gr.Column():
                boxes_seq_usage_file_url = f'file={os.path.join(os.path.dirname(__file__), f"examples/boxes_seq_explanation.jpg")}'
                gr.HTML(
f"""
...
"""
            ...
            with gr.Column():
                ...
                with gr.Accordion("Parameters", open=False):
                    with gr.Row():
                        ...
                        max_length = gr.Slider(0, 4096, value=512, step=1.0, label="max length", interactive=True)
                ...
                with gr.Column():
                    ...
                    with gr.Group():
                        if args.base_model == 'llama':
                            default_spot_cap_template = 'Provide a comprehensive description of the image <image> and specify the positions of any mentioned objects in square brackets.'
                        else:
                            default_spot_cap_template = "详述图片<image>中的事件，并提供谈论的对象的边界框信息。"
                        template = gr.Textbox(label='Template', show_label=True, lines=1, interactive=False,
                                              value=default_spot_cap_template)
        ...
        with gr.Column(visible=True) as example_SpotCap:
            _examples_cap_raw = [
                os.path.join(os.path.dirname(__file__), 'examples/proposal.jpg'),
                os.path.join(os.path.dirname(__file__), 'examples/water_question.jpg'),
                os.path.join(os.path.dirname(__file__), 'examples/fishing.jpg'),
                os.path.join(os.path.dirname(__file__), 'examples/ball.jpg'),

                os.path.join(os.path.dirname(__file__), 'examples/banana_phone.png'),
                os.path.join(os.path.dirname(__file__), "examples/airplane.jpg"),
                os.path.join(os.path.dirname(__file__), 'examples/baseball.png'),
            ]
        ...
        with gr.Column(visible=False) as example_vqabox:
            _examples_vqabox_parsed = [
                ...
                [
                    os.path.join(os.path.dirname(__file__), 'examples/woman_door.jpg'),
                    "Which one is the woman's reflection in the mirror?<boxes>",
                    '[[0,1]]',
                    [(770, 138, 1024, 752), (469, 146, 732, 744)],
                ],
                [
                    os.path.join(os.path.dirname(__file__), 'examples/man.jpg'),
                    "What is the person<boxes> scared of?",
                    '[[0]]',
                    [(148, 99, 576, 497)],
                ],
                [
                    os.path.join(os.path.dirname(__file__), "examples/giraffes.jpg"),
                    "How many animals in the image?",
                    "",
                    [],
                ],
                [
                    os.path.join(os.path.dirname(__file__), "examples/dog_selfcontrol.jpg"),
                    "Is this dog on a lead held by someone able to control it?",
                    "",
                    [],
                ],
                [
                    os.path.join(os.path.dirname(__file__), 'examples/wet_paint1.jpg'),
                    'What does the board say?',
                    '',
                    [],
                ],
                [
                    os.path.join(os.path.dirname(__file__), 'examples/g2.jpg'),
        ...
        with gr.Column(visible=False) as example_vqa:
            _examples_vqa_parsed = [
                [
                    os.path.join(os.path.dirname(__file__), 'examples/food-1898194_640.jpg'),
                    "QUESTION: Which of the following is meat?\nOPTION:\n(A) <boxes>\n(B) <boxes>\n(C) <boxes>\n(D) <boxes>",
                    '[[0],[1],[2],[3]]',
                    [[20, 216, 70, 343], [8, 3, 187, 127], [332, 386, 424, 494], [158, 518, 330, 605]],
                ],
                [
                    os.path.join(os.path.dirname(__file__), 'examples/potato.jpg'),
                ...
                [
                    os.path.join(os.path.dirname(__file__), 'examples/staircase-274614_640.jpg'),
                    "Is this a sea snail?",
                    '',
                    [],
                ],
        ...
        with gr.Column(visible=False) as example_rec:
            gr.Examples(
                examples=[
                    [
                        os.path.join(os.path.dirname(__file__), "examples/rec_bear.png"),
                        "a brown teddy bear with a blue bow",
                        [],
                    ],
                    [
                        os.path.join(os.path.dirname(__file__), "examples/bear-792466_1280.jpg"),
        ...
        with gr.Column(visible=False) as example_reg:
            gr.Examples(
                examples=[
                    [
                        os.path.join(os.path.dirname(__file__), "examples/fruits.jpg"),
                        "[[0]]",
                        [[833, 527, 646, 315]],
                    ],
                    [
                        os.path.join(os.path.dirname(__file__), "examples/bearhat.png"),
                        "[[0]]",
                        [[48, 49, 216, 152]],
                    ],
                    [
                        os.path.join(os.path.dirname(__file__), "examples/oven.jpg"),
                        "[[0]]",
                        [[1267, 314, 1383, 458]],
                    ],
                ],
        ...
        def change_textbox(choice):
            ...
            intern_task_template = {
                "SpotCap": "详述图片<image>中的事件，并提供谈论的对象的边界框信息。",
                "Cap": "请简单明确地描述一下这个图像<image>的内容。",
                "GCoT": "解析图片 <image> 并对问题 “<question>” 给出答案。确保提供详细的推理过程，并用方括号标出提到的对象的边界框。",
                "VQA": "查看图像<image>后，请直接回答'<question>'。",
                "REC": "<image>图像中<expr>的坐标是什么？",
                "REG": "对于给定的图像 <image>，提供区域 <boxes> 的有区分度的描述",
                "GC": "给我描述一下图像 <image> 中的区域 <boxes>",
                "Advanced": "<question>",
            }

            task_template = llama_task_template if args.base_model == 'llama' else intern_task_template
        ...
        def submit_step1(state, template, raw_user_input, boxes_seq, chatbot, do_sample, max_length, top_p, temperature):
            ...
            try:
                ...
                obj = ds.get_raw_item()
                if 'image' in obj:
                    obj['img_base64'] = pil_to_base64(obj['image'])
                    del obj['image']
